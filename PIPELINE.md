...
**Default data source:** `job_scraping/output/english_jobs.csv` (config.JOBS_SCRAPING_CSV) — 12 months of job postings. Run `preprocess_jobs_pipeline.py` to produce pipeline-ready files.

| File | Location | Purpose |
|------|----------|---------|
| `jobs_sentences.csv` | `DATA/preprocessing/data_prepared/` (config.PIPELINE_INPUT_CSV) | Pipeline input: job_id, sentence_text, job_date |
| `jobs_metadata.csv` | `DATA/preprocessing/data_prepared/` (config.PREPROCESS_OUTPUT_DIR) | job_id → job_date mapping for enrich_with_dates |
...
- **DATA/preprocessing/data_prepared/**: Preprocessed job data (config.PREPROCESS_OUTPUT_DIR)
- **job_scraping/output/**: Raw scraped jobs (config.JOBS_SCRAPING_CSV)
...
| enrich_with_dates fails | jobs_metadata.csv at config.PREPROCESS_OUTPUT_DIR; or pass --jobs_meta path |
...
### Preprocessing & Data Paths (2025)
- **Default data source:** `job_scraping/output/english_jobs.csv` (config.JOBS_SCRAPING_CSV)
- `preprocess_jobs_pipeline.py` defaults: `--input` = JOBS_SCRAPING_CSV, `--output_dir` = PREPROCESS_OUTPUT_DIR
- `pipeline.py` INPUT_CSV from config.PIPELINE_INPUT_CSV
- `run_with_job_scraping.py`: One-step preprocess + pipeline using job_scraping data
- `enrich_with_dates.py` defaults to config.PREPROCESS_OUTPUT_DIR/jobs_metadata.csv
...
### LLM-Only Knowledge & Extraction Design (2025)
- **Direction A:** Knowledge output from LLM only; BERT knowledge passed to LLM as anti-hallucination context but not fused.
- **Design:** BERT per sentence (128-token limit); LLM on full job description (full context).
- **Competency anti-hallucination:** Prompt rule + post-validation filter for `related_skills` in generate_competencies.py.
- **JobBERT token limit:** Preprocess splits on bullet boundaries and paragraph breaks; max_len reduced to 600 chars.
